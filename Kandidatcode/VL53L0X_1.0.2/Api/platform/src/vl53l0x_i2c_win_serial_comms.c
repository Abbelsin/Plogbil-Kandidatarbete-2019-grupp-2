...
//Write to multiple bytes to a sensor
int32_t VL53L0X_write_multi(uint8_t address, uint8_t reg, uint8_t *pdata, int32_t count)
{
    int32_t status = STATUS_OK;
	//Write an amount of data in blocking mode to a specific memory address
	HAL_I2C_Mem_Write(&hi2c1, address, reg, I2C_MEMADD_SIZE_8BIT, pdata, count, 50000);
	
    return status;
}

//Read multiple bytes from a sensor
int32_t VL53L0X_read_multi(uint8_t address, uint8_t index, uint8_t *pdata, int32_t count)
{
    int32_t status = STATUS_OK;
    HAL_I2C_Mem_Read(&hi2c1, address, index, I2C_MEMADD_SIZE_8BIT, pdata, count, 50000);
